...
class GameListView(LoginRequiredMixin, generic.ListView):
    ...
    def _prepare_context(self, context: dict) -> dict:
        ...
        context['selected_games'] = SelectedGamesService().get_user_selected_games(self.request.user)
    ...
    def get_queryset(self):
        ...
        if game or tags:
            ...
            for tag in tags:
                object_list = GameService.filter_games_which_contains_tag(tag, object_list)
...
class RecommendationListView(LoginRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        ...
        Return the list of items for this view.
        """
        recommendation_objects = RecommendationService.get_all_recommendations(self.request.user)
...
class OpinionFormView(LoginRequiredMixin, SuccessMessageMixin, generic.FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        recommendation = RecommendationService().get_recommendation_by_id(id=self.kwargs['pk'])
...
def game_search(request: HttpRequest) -> JsonResponse:
    ...
    if request.method == 'GET':
        ...
        payload = []
        if game:
            games_objs = GameService.filter_games_which_contains_string(searched_string=game)[:10]
...
def _retrieve_attributes_from_request(request: HttpRequest) -> tuple[str, str, str, list, list]:
    """
...
def create_recommendation(request: HttpRequest):
    ...
    """
    if request.method == 'POST':
        selected_games_obj = SelectedGamesService.get_selected_games_object_by_user(user=request.user)
        if not selected_games_obj.selected_games.all():
            return redirect("polecacz:selected_games")

        recommendation_object = RecommendationService.create_recommendation(user=request.user)
...
def _create_recommendation_using_tags(used_games_ids: list[str], tag_list: list[str]) -> QuerySet:
    """
